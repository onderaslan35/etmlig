...
export default function TffPuanDurumuPage() {
  ...
  const loadLeaderboard = async () => {
    try {
      ...
      // Artık manuel kod listesi yok, veriler veritabanından çekilecek!
      ...
      // 🔴 1000 LİMİT KIRICI VE EKSİKSİZ TAHMİN TOPLAYICI (Sıralama Eklendi)
      ...
      if (dbMatches) {
        ...
        Object.values(uniqueMatches).forEach(dbMatch => {
          if (
            ...
            // 🔴 İŞTE TFF'DEKİ HATAYI ÇÖZEN KISIM! KLON SİLİCİ YOK, ARTIK 9 KİŞİNİN 9'UNU DA SAYACAK!
            let points = 1;
      ...
      // GEÇMİŞ (tff_weekly_points) + 5. HAFTA BİRLEŞTİRME
      ...
      console.log("TFF verisi çekilirken hata oluştu");
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 text-slate-100 flex flex-col items-center">
      ...
      <div className="flex flex-col items-center text-center mb-5 mt-1">
        <h1 className="text-xl md:text-2xl font-extrabold text-amber-400 uppercase drop-shadow-md">
          TFF PUAN DURUMU
        </h1>
      </div>
              ...
              {[1, 2, 3, 4, 5].map(num => (
                <button
                  ...
                  className={`w-12 h-10 flex items-center justify-center rounded-lg font-bold text-sm transition-all ${
                    activeTab === `w${num}`
                      ? 'bg-[#f59e0b] text-black border-amber-400 border'
