...
// TFF ORGANİZASYONLARI GÜNCEL PUAN DURUMU (TOPLAM VE 3. HAFTA VERİSİ)
const tffStandingsData = [
  ...
  { id: "262754", name: "OSMAN ALİ AYDIN 🏆", puan: 3 },
  { id: "262705", name: "AHMET BİRCAN 🏆", puan: 3 },
...
export default function TffPuanDurumuPage() {
  ...
  useEffect(() => {
    if (activeTab === 'total' || activeTab === 'week3') {
      // 3. Hafta ve Toplam Puan aynı veriyi gösteriyor
      const sorted = [...tffStandingsData].sort((a, b) => (b.puan || 0) - (a.puan || 0));
      setTableRows(sorted);
    } else {
      let currentData: any[] = [];
  ...
  const getActiveTabTitle = () => {
    if (activeTab === 'total') return 'TFF ORGANİZASYONLARI TOPLAM PUAN DURUMU';
    ...
    <div className="max-w-5xl mx-auto p-4 text-slate-100 flex flex-col items-center">
      ...
      <div className="flex flex-col items-center text-center mb-5 mt-1">
        <h1 className="text-xl md:text-2xl font-extrabold text-center text-amber-400 tracking-wider uppercase">
          TFF ORGANİZASYONLARI PUAN DURUMU
        </h1>
      </div>

      {/* 2. BUGÜNÜN MÜSABAKALARI - CAROUSEL */}
