...
export default function DfoPuanDurumuPage() {
  ...
  const loadLeaderboard = async () => {
    try {
      ...
      // 🔴 EKMEL KANUNU: DFO PUAN TABLOSUNDAN (dfo_weekly_points) DOĞRU VERİ ÇEKİLDİ 🔴
      ...
      if (dbMatches) {
        ...
        Object.values(uniqueMatches).forEach(dbMatch => {
          ...
          if (weekNum >= 5 && weekNum <= 38 && dbMatch.home_score && dbMatch.home_score !== '-' && dbMatch.away_score && dbMatch.away_score !== '-') {
            ...
            // 🔴 EKMEL KANUNU: DFO SADECE AVRUPA MAÇLARINI DİKKATE ALIR
            if (isTff) return; 
      ...
      const baseList = Object.keys(playersList).map(id => {
        ...
        for (let w = 5; w <= highestWeekFound; w++) {
            ...
            // 🔴 EKMEL KANUNU: DFO'YA ASLA BONUS EKLENMEZ, SADECE HAM MAÇ PUANI 🔴
