...
export default function DfoPuanDurumuPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 text-slate-100 flex flex-col items-center">
      {/* 1. LOGO (MASTER İLE BİREBİR BOYUT) */}
      ...
      {/* 3. SEKMELER VE HAFTA SEÇİMLERİ (MASTER İLE BİREBİR AYNI TASARIM) */}
      <div className="w-full flex flex-col items-center mb-6">
        ...
        {/* HAFTA BUTONLARI IZGARASI (MASTER İLE BİREBİR AYNI KIRMIZI BUTONLAR) */}
            ...
            {row1Weeks.map((weekNum) => {
              ...
              return (
                <button
                  ...
                  title={`${weekNum}. Hafta DFO Puan Durumu`}
                >
                  {weekNum}
                </button>
              );
      ...
      {/* PUAN TABLOSU (HİÇBİR VERİYE VE HESAPLAMAYA DOKUNULMADI) */}
      <div className="w-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
        {tableRows.length > 0 ? (
              ...
              <thead className="bg-slate-950 text-slate-400 uppercase text-xs border-b border-slate-800">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right">
                    {activeTab === 'total' ? 'TOPLAM DFO PUANI' : 'HAFTALIK DFO PUANI'}
                  </th>
                </tr>
              </thead>
          ...
          <div className="py-12 text-center text-slate-500 font-medium">
            ⏳ {activeTab.replace('week', '')}. Haftanın DFO verileri bulunamadı.
          </div>
        )}
      </div>
    </div>
