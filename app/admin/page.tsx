...
// Komutanım, ekran görüntüsündeki logoları anında yansısın diye buraya ekledim. 
// Dilerseniz Maç Arşivi sayfanızdaki tüm takımları buraya ekleyebilirsiniz!
...
export default function AdminRadarPortal() {
    ...
    const loadBulletinData = async () => {
      ...
      if (activeTab === 'bulletin') {
        ...
        if (data && data.length > 0) {
          ...
          setBulletinMatches(Array.from({ length: 24 }, (_, i) => ({ match_index: i + 1, category: 'TÜRKİYE SÜPER LİG', match_date: newDates[0], match_time: '21:00', home_team: '', away_team: '' })));
        }
      }
    };
    loadBulletinData();
  ...
  const handleAddNewPlayer = async (e: React.FormEvent) => { e.preventDefault(); alert("Kayıt kapalıdır."); };
  const handleBanishPlayer = async (userId: string, userName: string) => { alert("Kayıt kapalıdır."); };
  ...
  const handleDeleteTeam = async (teamName: string) => {
    ...
    try {
       ...
       if (error) throw error; alert(`✅ Silindi!`); fetchAllTeamsFromDB(); 
    } catch (err: any) { alert("❌ Hata: " + err.message); }
  };
  ...
  const isTffMatchCheck = (category: string) => TFF_CATEGORIES.includes(category.toUpperCase());
  ...
  const getAvailableTeams = (currentIndex: number, isHome: boolean) => {
    ...
    return Object.keys(teamLogosMap).filter(t => cleanTeamName(t) !== cleanTeamName(opponent)).sort((a,b) => a.localeCompare(b,'tr'));
  };

  const handleBulletinChange = (index: number, field: string, value: string) => {
    const newMatches = [...bulletinMatches]; (newMatches[index] as any)[field] = value;
    if (field === 'category') { newMatches[index].home_team = ''; newMatches[index].away_team = ''; }
    setBulletinMatches(newMatches);
  };

  const copyDateTimeToAll = () => {
    const firstDate = bulletinMatches[0].match_date; const firstTime = bulletinMatches[0].match_time;
    if(!firstDate || !firstTime) return alert("Önce 1. maçın tarih ve saatini doldurun!");
    setBulletinMatches(bulletinMatches.map(m => ({ ...m, match_date: firstDate, match_time: firstTime })));
  };

  const saveBulletinToDB = async () => {
    if (bulletinMatches.some(m => !m.home_team.trim() || !m.away_team.trim())) { if(!window.confirm("Bazı takımlar seçilmemiş. Yayınlamak istiyor musun?")) return; }
    setIsPublishing(true);
    try {
      const payload = bulletinMatches.map(m => {
        const hTeam = cleanTeamName(m.home_team); const aTeam = cleanTeamName(m.away_team);
        return {
          week_num: bulletinWeek, match_index: m.match_index, category: m.category, match_date: m.match_date, match_time: m.match_time,
      ...
      if (error) throw error; alert(`✅ BAŞARILI!`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-3 sm:p-6 font-sans pb-24">
      <div className="max-w-7xl mx-auto pt-6">
        <div className="flex flex-col lg:flex-row gap-4 mb-8 bg-slate-900/50 p-3 rounded-2xl border border-slate-800 shadow-xl overflow-x-auto custom-scrollbar flex-wrap">
           ...
           {userRole === 'master' && (
             <>
               <button onClick={() => setActiveTab('bulletin')} className={`flex-1 min-w-[160px] py-3 rounded-xl font-black text-xs lg:text-sm tracking-widest ${activeTab === 'bulletin' ? 'bg-indigo-600 text-white' : 'bg-slate-800/50 text-slate-400'}`}>🛠️ BÜLTEN</button>
               ...
               <button onClick={() => setActiveTab('players')} className={`flex-1 min-w-[160px] py-3 rounded-xl font-black text-xs lg:text-sm tracking-widest ${activeTab === 'players' ? 'bg-fuchsia-600 text-white' : 'bg-slate-800/50 text-slate-400'}`}>👥 YARIŞMACILAR</button>
               <button onClick={() => setActiveTab('teams')} className={`flex-1 min-w-[160px] py-3 rounded-xl font-black text-xs lg:text-sm tracking-widest ${activeTab === 'teams' ? 'bg-cyan-600 text-white' : 'bg-slate-800/50 text-slate-400'}`}>🛡️ TAKIM YÖNETİMİ</button>
             </>
           )}
        </div>
        ...
        {activeTab === 'live' && (
          <div className="animate-fade-in">
            ...
            {userRole === 'master' && (
            <div className="mb-8 p-5 bg-gradient-to-r from-blue-950/80 via-slate-900 to-indigo-950/80 border border-blue-500/30 rounded-2xl">
                <h2 className="text-center font-black text-blue-400 text-sm tracking-widest uppercase mb-4 flex items-center justify-center gap-2">🏆 {selectedLiveWeek}. HAFTA CANLI LİDERLİK RADARI</h2>
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                    <div className="bg-slate-950/80 border border-emerald-500/50 rounded-xl p-3 w-full max-w-xs flex flex-col items-center">
                        <span className="text-emerald-400 text-[10px] font-bold tracking-widest mb-1">🔥 PUAN LİDERİ</span>
                        <span className="text-white font-black text-sm uppercase">{weeklyStats.pointsLeader ? `${mergedPlayers[weeklyStats.pointsLeader]}` : 'LİDER YOK'}</span>
                    </div>
                    <div className="bg-slate-950/80 border border-amber-500/50 rounded-xl p-3 w-full max-w-xs flex flex-col items-center">
                        <span className="text-amber-400 text-[10px] font-bold tracking-widest mb-1">⚽ SKOR KRALI</span>
                        <span className="text-white font-black text-sm uppercase">{weeklyStats.scoreLeader ? `${mergedPlayers[weeklyStats.scoreLeader]}` : 'KRAL YOK'}</span>
                    </div>
                </div>
            </div>
            )}
        ...
        {activeTab === 'bulletin' && userRole === 'master' && (
          <div className="animate-fade-in">
             <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-4"><h2 className="text-xl font-black text-indigo-400">🏭 BÜLTEN FABRİKASI</h2><div className="flex items-center gap-3"><select value={bulletinWeek} onChange={e => setBulletinWeek(Number(e.target.value))} className="bg-indigo-950 text-indigo-300 font-bold px-3 py-1 rounded"><option value={4}>4. HAFTA</option><option value={5}>5. HAFTA</option><option value={6}>6. HAFTA</option></select></div></div>
                      ...
                      <tbody>
                         {bulletinMatches.map((m, idx) => (
                            <tr key={m.match_index} className="border-b border-slate-800">
                               ...
                               <td className="p-2"><select value={m.category} onChange={e=>handleBulletinChange(idx,'category',e.target.value)} className="w-full bg-slate-950 px-2 py-1 rounded"><option value="">Kategori Seçin</option>{dynamicCategoriesList.map(c=><option key={c} value={c}>{c}</option>)}</select></td>
        ...
        {activeTab === 'players' && userRole === 'master' && (
          <div className="animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-800 pb-4"><div className="text-center sm:text-left"><h1 className="text-2xl font-black text-fuchsia-400 tracking-tight flex items-center justify-center sm:justify-start gap-3 uppercase"><span className="text-3xl">👥</span> YARIŞMACI YÖNETİMİ</h1><p className="text-slate-400 text-sm mt-1">Kayıtlı 54 Aslan Parçası sisteme kalıcı olarak mühürlenmiştir.</p></div></div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl h-fit opacity-50 cursor-not-allowed">
                 <div className="flex items-center justify-between mb-6 border-b border-slate-800 pb-4"><h2 className="text-lg font-black text-fuchsia-400 flex items-center gap-2"><span className="text-xl">➕</span> YENİ ASLAN PARÇASI EKLE</h2></div>
                 <form onSubmit={handleAddNewPlayer} className="flex flex-col gap-5">
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">6 HANELİ YARIŞMACI ID</label><input type="text" disabled placeholder="Örn: 262888" className="w-full bg-slate-950 border border-slate-700 text-slate-500 px-4 py-3 rounded-xl outline-none font-black tracking-widest shadow-inner cursor-not-allowed" /></div>
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">İSİM SOYİSİM</label><input type="text" disabled placeholder="Örn: SİNAN ENGİN" className="w-full bg-slate-950 border border-slate-700 text-slate-500 px-4 py-3 rounded-xl outline-none font-black tracking-widest uppercase shadow-inner cursor-not-allowed" /></div>
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">GİRİŞ ŞİFRESİ</label><input type="text" disabled placeholder="Örn: 19030" className="w-full bg-slate-950 border border-slate-700 text-slate-500 px-4 py-3 rounded-xl outline-none font-black tracking-widest shadow-inner cursor-not-allowed" /></div>
                    <button type="submit" disabled className="mt-4 bg-slate-700 text-slate-400 font-black tracking-widest py-4 rounded-xl flex justify-center items-center gap-2 cursor-not-allowed">SİSTEME KAYIT KAPALIDIR</button>
                 </form>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
                 <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-4"><h2 className="text-lg font-black text-rose-500 flex items-center gap-2"><span className="text-xl">⚖️</span> 54 ASLAN PARÇASI (TÜM LİSTE)</h2><span className="bg-slate-950 text-emerald-400 px-3 py-1 rounded-lg text-xs font-bold border border-emerald-800">Kalıcı Liste</span></div>
                 <div className="flex flex-col gap-2 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                    {dbPlayersList.length === 0 ? ( <div className="text-center py-8 text-slate-500 italic">Veritabanında kayıtlı kimse yok.</div> ) : ( dbPlayersList.map(p => ( <div key={p.id} className="bg-slate-950/80 border border-slate-800 p-3 rounded-xl flex justify-between items-center group hover:border-slate-600 transition-colors"><div className="flex flex-col"><span className="font-black text-slate-200 text-sm uppercase tracking-wide">{p.full_name}</span><span className="text-[10px] font-bold text-slate-500 tracking-widest mt-0.5">ID: {p.user_id}</span></div><button onClick={() => handleBanishPlayer(p.user_id, p.full_name)} className="bg-slate-800 text-slate-500 border border-slate-700 px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest cursor-not-allowed" title="Silme işlemi kapalıdır">🔒 KORUMALI</button></div> )) )}
                 </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'teams' && userRole === 'master' && (
          <div className="animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b border-slate-800 pb-4"><div className="text-center sm:text-left"><h1 className="text-2xl font-black text-cyan-400 tracking-tight flex items-center justify-center sm:justify-start gap-3 uppercase"><span className="text-3xl">🛡️</span> TAKIM CEPHANELİĞİ</h1></div></div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl h-fit">
                 <div className="flex items-center justify-between mb-6 border-b border-slate-800 pb-4"><h2 className="text-lg font-black text-cyan-400 flex items-center gap-2"><span className="text-xl">➕</span> YENİ TAKIM EKLE</h2></div>
                 <form onSubmit={handleAddNewTeam} className="flex flex-col gap-5">
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">TAKIM ADI</label><input type="text" value={newTeamName} onChange={e => setNewTeamName(e.target.value)} placeholder="Örn: BALÇOVA YAŞAM SPOR" className="w-full bg-slate-950 border border-slate-700 text-slate-200 px-4 py-3 rounded-xl outline-none focus:border-cyan-500 font-black tracking-widest uppercase shadow-inner" /></div>
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">BAĞLI OLDUĞU LİG (İSTER SEÇ, İSTER YAZ)</label><input list="leagueOptions" value={newTeamLeague} onChange={e => setNewTeamLeague(e.target.value.toUpperCase())} placeholder="Örn: TFF 2.LİG veya listeden seçin..." className="w-full bg-slate-950 border border-slate-700 text-slate-300 px-4 py-3 rounded-xl outline-none focus:border-cyan-500 font-bold tracking-widest shadow-inner uppercase" /><datalist id="leagueOptions">{teamLeagueOptions.map(l => <option key={l} value={l} />)}</datalist></div>
                    <div><label className="block text-xs font-bold text-slate-400 tracking-widest mb-1.5 ml-1">LOGO LİNKİ (VİKİPEDİ / URL)</label><input type="text" value={newTeamLogo} onChange={e => setNewTeamLogo(e.target.value)} placeholder="Örn: https://upload.wikimedia.org/.../logo.svg" className="w-full bg-slate-950 border border-slate-700 text-amber-400 px-4 py-3 rounded-xl outline-none focus:border-cyan-500 font-mono text-sm tracking-tight shadow-inner" /></div>
                    <button type="submit" disabled={isTeamLoading} className="mt-4 bg-cyan-600 hover:bg-cyan-500 text-white font-black tracking-widest py-4 rounded-xl transition-all shadow-[0_0_15px_rgba(8,145,178,0.4)]">TAKIMI SİSTEME KAYDET</button>
                 </form>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
                 <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-4"><h2 className="text-lg font-black text-rose-500 flex items-center gap-2"><span className="text-xl">📋</span> VERİTABANINDAKİ TAKIMLAR</h2><span className="bg-slate-950 text-slate-400 px-3 py-1 rounded-lg text-xs font-bold border border-slate-800">{dbTeamsList.length} Takım Kayıtlı</span></div>
                 <div className="flex flex-col gap-2 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                    {dbTeamsList.length === 0 ? ( <div className="text-center py-8 text-slate-500 italic">Veritabanında kayıtlı takım yok.</div> ) : ( dbTeamsList.map(t => { const safeName = cleanTeamName(t.name || t.team_name); const safeCat = t.category || t.league; return ( <div key={t.id} className="bg-slate-950/80 border border-slate-800 p-2 sm:p-3 rounded-xl flex justify-between items-center group hover:border-cyan-900/50 transition-colors gap-2"><div className="flex items-center gap-3 overflow-hidden"><div className="w-10 h-10 bg-slate-900 rounded border border-slate-700 flex items-center justify-center flex-shrink-0 p-1"><img src={t.logo_url} alt={safeName} className="max-w-full max-h-full object-contain drop-shadow-md" /></div><div className="flex flex-col overflow-hidden"><span className="font-black text-slate-200 text-[11px] sm:text-xs uppercase tracking-wide truncate">{safeName}</span><span className="text-[9px] sm:text-[10px] font-bold text-cyan-500/70 tracking-widest mt-0.5 truncate">{safeCat}</span></div></div><button onClick={() => handleDeleteTeam(t.name || t.team_name)} className="flex-shrink-0 bg-rose-950/80 hover:bg-rose-600 text-rose-400 hover:text-white border border-rose-900/50 hover:border-rose-500 px-2 py-1.5 rounded-lg text-[9px] font-black tracking-widest transition-all">❌ SİL</button></div> )}) )}
                 </div>
              </div>
            </div>
          </div>
