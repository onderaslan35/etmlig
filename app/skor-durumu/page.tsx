                ...
                <tbody className="divide-y divide-[#1e293b]">
                  {currentList.map((row: any, idx: number) => {
                    ...
                    <tr key={idx} className="hover:bg-[#0f172a]/40 transition-colors">
                      ...
                      <td className={`pr-2 md:pr-4 pl-1 py-3 text-center font-bold text-sm align-middle ${activeTab === 'MASTER' ? 'text-amber-500' : activeTab === 'DFO' ? 'text-blue-400' : 'text-red-500'}`}>
                        <div className="flex flex-col items-center justify-center gap-1">
                          <span>{row.totalScore}</span>
                          {/* 🔥 İŞTE CANLI SKOR (TAM İSABET) ROZETİ 🔥 */}
                          {row.liveBonus > 0 && (
